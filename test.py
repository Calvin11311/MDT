...
import torch
import argparse
from basicsr.models.archs.mdt_arch import  mdt
...
def _eval(model, args):
    ...
    with torch.no_grad():
        ...
        for iter_idx, data in tqdm(enumerate(dataloader)):
            ...
            bs=1
            dist = torch.tensor(np.array([0.5, 0.5, 0.5, 0.5]).reshape(1, 4)).float().repeat([bs,1]).cuda()
            # pred = model(input_img)
            pred = model(input_img,dist)
...
if __name__ == '__main__':
    ...
    # Directories
    parser.add_argument('--model_name', default='mdt', type=str)
    parser.add_argument('--data_dir', type=str, default='./')

    # Test
    parser.add_argument('--test_model', type=str, default='./pretrain_model/net_g_GoPro_HIDE.pth')
    parser.add_argument('--save_image', type=bool, default=True, choices=[True, False])
    parser.add_argument('--result_dir', type=str, default='./results/mdt/GoPro')
    args = parser.parse_args()
    # args.result_dir = os.path.join('results/', args.model_name, 'GoPro_mytrain/')
    print(args)
    main(args)
